...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
    ...
    if (!event) return { title: 'Predstavenie nenájdené' };

    return {
        title: `${event.title} - ${event.category === 'skola-ludus' ? 'Škola' : 'Divadlo'} LUDUS`,
        description: event.subtitle || event.description?.slice(0, 160) || `Pozrite si predstavenie ${event.title} v Divadle LUDUS Bratislava.`,
        openGraph: {
            title: event.title,
            description: event.subtitle || event.description?.slice(0, 160),
            images: event.image_path ? [getImageUrl(event.image_path)] : [],
        },
    };
}
