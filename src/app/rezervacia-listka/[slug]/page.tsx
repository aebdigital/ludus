...
export default function TicketReservationPage({ params }: { params: Promise<{ slug: string }> }) {
            ...
            const response = await fetch('/api/contact', {
                ...
                body: JSON.stringify({
                    ...
                    reservationDetails: {
                        ...
                        'Čas': event.time,
                        'Miesto': event.venue,
                        'Počet lístkov': formData.ticket_count,
                        'Meno': formData.contact_person,
                        'Email': formData.email,
                        'Telefón': formData.phone,
                        'Poznámka': formData.message
                    }
                }),
            });
        ...
        return (
            <div className="min-h-screen pt-40 px-6 max-w-3xl mx-auto text-center">
                <div className="bg-green-50 p-10 rounded-2xl border border-green-200">
                    ...
                    <p className="text-lg text-green-700">Vaša rezervácia bola úspešne odoslaná. Lístky Vás budú čakať na mieste.</p>
    ...
    return (
        <main className="min-h-screen pt-32 pb-20 px-6 bg-gray-50">
            <div className="max-w-xl mx-auto bg-white p-8 md:p-12 rounded-2xl shadow-xl">
                <h1 className="text-3xl md:text-4xl font-bold mb-4 font-heading text-center" style={{ fontFamily: 'var(--font-heading)' }}>
                    Záväzná rezervácia lístka
                </h1>

                <div className="mb-8 p-4 bg-yellow-50 text-yellow-800 text-center text-sm rounded-lg border border-yellow-200 font-medium">
                    Platba za lístky je momentálne možná len na mieste v hotovosti.
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    {/* Counts */}
                    <div>
                        <label className="block text-sm font-bold mb-1">Počet lístkov</label>
                        <input
                            type="number"
                            name="ticket_count"
                            required
                    ...
                    <div>
                        <label className="block text-sm font-bold mb-1">Vaša správa (prosím uveďte, ak ste učiteľ*ka alebo študent*ka)</label>
                        <textarea
                            name="message"
                            rows={4}
