                ...
                <AnimatePresence initial={false} mode="popLayout">
                    {visibleItems.map((play, i) => (
                        <motion.div
                            ...
                            className="flex items-start gap-2 hover:text-[#f47f44] transition-colors leading-tight"
                        >
                            <span className="text-[#f47f44]">•</span>
                            <span>{play}</span>
                        </motion.div>
                    ))}
                </AnimatePresence>
            ...
            {items.length > initialCount && (
                <div className="mt-8 text-center relative z-10">
                    <button
                        ...
                        className="inline-flex items-center gap-2 bg-black text-white px-6 py-3 rounded-full hover:bg-[#f47f44] hover:text-black transition-all duration-300 font-bold uppercase tracking-wider text-sm shadow-md"
                    >
                        {isExpanded ? (
                            <>
